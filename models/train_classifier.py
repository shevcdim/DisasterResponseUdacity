...
def load_data(database_filepath):
    '''
    connect to DB on database_filepath, read table drmsg into pandas DF
    extract messages content into X   
    categories values into y and category labels into category_label
    '''
    
    engine = create_engine('sqlite:///{}'.format(database_filepath))
    df = pd.read_sql_table(table_name='drmsg', con=engine)
    #print(df.columns)
    
    X = df.message.values
    y= df.iloc[:, ~df.columns.isin(['id', 'message', 'original', 'genre'])]
